import { newVideoMessage } from "./eventReceiver";
...
export const createThumbnail = async (videoID: number): Promise<void> => {
  ...
  return new Promise((resolve, reject) => {
    ffmpeg()
      ...
      .on("progress", ({ percent: p }: ffmpegProgress) => {
        p && ingestJob.setProgress(p).then();
      })
      .on("error", (e: Error) =>
        ingestJob.setState("Failed", e.message).then(reject)
      )
      .run();
  });
};
...
export const createBroadcast = async (videoID: number): Promise<void> => {
  ...
  return new Promise((resolve, reject) => {
    ffmpeg()
      ...
      .on("error", (e: Error) => {
        ingestJob.setState("Failed", e.message).then(() => {
          reject(e);
        });
      })
      .run();
